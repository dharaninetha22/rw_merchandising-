        ...
        <Grid container spacing={4} justifyContent="flex-start">
          {filteredProducts.map((product) => (
            <Grid item key={product.id} xs={12} sm={6} md={3}>
              ...
              >
                <Box sx={{ position: "relative" }} onMouseEnter={() => handleMouseEnter(product.id.toString())} onMouseLeave={handleMouseLeave}>
                  <CardMedia component="img" image={hoveredProductId === product.id.toString() ? product.hoverImage : product.image} alt={product.title} sx={{ objectFit: "full" }} />
                  <Typography sx={{ position: "absolute", top: 10, left: 10, backgroundColor: "primary.main", color: "white", padding: "1px 1px", borderRadius: "8px", fontSize: "12px" }}>
                    {product.discount}
                  </Typography>
